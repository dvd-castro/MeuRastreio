import br.com.davidcastro.data.model.TrackingModel
import com.google.gson.Gson

object TrackingResponseRealMock {

    private const val mockJsonString = "{\n" +
            "\"codigo\": \"NL276547976BR\",\n" +
            "\"host\": \"vc\",\n" +
            "\"eventos\": [\n" +
            "{\n" +
            "    \"status\": \"Objeto entregue ao destinatário\",\n" +
            "    \"local\": \"FORTALEZA - CE\",\n" +
            "    \"data\": \"07/12/2022\",\n" +
            "    \"hora\": \"14:14\",\n" +
            "    \"subStatus\": [\n" +
            "    \"De:  FORTALEZA - CE - \"\n" +
            "    ]\n" +
            "},\n" +
            "{\n" +
            "    \"status\": \"Objeto saiu para entrega ao destinatário\",\n" +
            "    \"local\": \"FORTALEZA - CE\",\n" +
            "    \"data\": \"07/12/2022\",\n" +
            "    \"hora\": \"09:07\",\n" +
            "    \"subStatus\": [\n" +
            "    \"De:  FORTALEZA - CE - \"\n" +
            "    ]\n" +
            "},\n" +
            "{\n" +
            "    \"status\": \"Objeto encaminhado\",\n" +
            "    \"local\": \"FORTALEZA - CE\",\n" +
            "    \"data\": \"30/11/2022\",\n" +
            "    \"hora\": \"07:59\",\n" +
            "    \"subStatus\": [\n" +
            "    \"De:  FORTALEZA - CE  -> Para:  Unidade de Distribui��o - FORTALEZA/CE\"\n" +
            "    ]\n" +
            "},\n" +
            "{\n" +
            "    \"status\": \"Objeto encaminhado\",\n" +
            "    \"local\": \"CURITIBA - PR\",\n" +
            "    \"data\": \"25/11/2022\",\n" +
            "    \"hora\": \"15:29\",\n" +
            "    \"subStatus\": [\n" +
            "    \"De:  CURITIBA - PR  -> Para:  Unidade de Tratamento - FORTALEZA/CE\"\n" +
            "    ]\n" +
            "},\n" +
            "{\n" +
            "    \"status\": \"Fiscalização aduaneira finalizada\",\n" +
            "    \"local\": \"CURITIBA - PR\",\n" +
            "    \"data\": \"25/11/2022\",\n" +
            "    \"hora\": \"14:33\",\n" +
            "    \"subStatus\": [\n" +
            "    \"De:  CURITIBA - PR - \"\n" +
            "    ]\n" +
            "},\n" +
            "{\n" +
            "    \"status\": \"Objeto recebido pelos Correios do Brasil\",\n" +
            "    \"local\": \"CURITIBA - PR\",\n" +
            "    \"data\": \"25/11/2022\",\n" +
            "    \"hora\": \"10:59\",\n" +
            "    \"subStatus\": [\n" +
            "    \"De:  CURITIBA - PR - \",\n" +
            "    \"<span class=\\\"minhasImportacoes\\\">Acesse o ambiente <a href=\\\"https://www.correios.com.br/encomendas-logistica/minhas-importacoes/minhas-importacoes\\\" target=\\\"_blank\\\">Minhas Importações</a></span>\"\n" +
            "    ]\n" +
            "},\n" +
            "{\n" +
            "    \"status\": \"Objeto postado\",\n" +
            "    \"local\": \"\",\n" +
            "    \"data\": \"20/11/2022\",\n" +
            "    \"hora\": \"06:16\",\n" +
            "    \"subStatus\": [\n" +
            "    \" \"\n" +
            "    ]\n" +
            "},\n" +
            "{\n" +
            "    \"status\": \"Objeto entregue ao destinatário\",\n" +
            "    \"local\": \"FORTALEZA - CE\",\n" +
            "    \"data\": \"07/12/2022\",\n" +
            "    \"hora\": \"14:14\",\n" +
            "    \"subStatus\": [\n" +
            "    \"De:  FORTALEZA - CE - \"\n" +
            "    ]\n" +
            "}\n" +
            "],\n" +
            "\"time\": 1.636,\n" +
            "\"quantidade\": 8,\n" +
            "\"servico\": \"\",\n" +
            "\"ultimo\": \"2022-12-07T17:14:00.000Z\"\n" +
            "}"

    fun getTrackingModelResponse(): TrackingModel = Gson().fromJson(mockJsonString, TrackingModel::class.java)
}